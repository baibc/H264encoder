...
unsigned char* encOutData =NULL;
typedef void *(*RunThread)(void *);
static void* pthreadCopyHandle(void *arg);
static void* pthreadSurfaceHandle(void *arg);
...
int exitEncPthreadFlag =0;
int exitSurPthreadFlag =0;
int exitCpyPthreadFlag =0;
typedef struct _copyBuffer {
    unsigned char* dataMem;
    int dataFlag;
}copyBuffer;
copyBuffer encBuf[6];

static void* pthreadEncoderHandle(void *arg)
{
		int k=0;
		while(1)
		{
			if(exitEncPthreadFlag ==1)
			{
				exitEncPthreadFlag =0;
				pthread_exit(NULL);
				break;
			}
			...
			for(k=0; k<6; k=k+1)
			{
				if(encBuf[k].dataFlag ==1)
				{
					libveENC_handle(&pVideoEnc, encBuf[k].dataMem, encOutData, &sps_pps_data, &bufferParam);
					nativeWindowRender(encBuf[k].dataMem, yuvSurface, picWidth, picHeight);
					encBuf[k].dataFlag =0;
					usleep(1);
				}
				usleep(200);
...
int dqbuf(int fps)
{
		...
		if(fps ==60)
		{
			if(bcount==1 || bcount==3 || bcount==6 || bcount==8 || bcount==10)
			{
				if(encBuf[bufCount].dataFlag ==0)
				{
					memcpy(encBuf[bufCount].dataMem, buffers[v4l2_buf.index].start, buffers[v4l2_buf.index].length);
					encBuf[bufCount].dataFlag =1;
				}
				if(++bufCount ==6)  bufCount=0;

			}else{}
			if(++bcount ==13)   bcount =1;
...
static int h264_config(int tmp)
{
		...
		h264Param.nBitrate = tmp*1024*1024; /* bps */
...
int main(int argc, char *argv[]) 
{
		...
		if( init( atoi(argv[1]), atoi(argv[2])) <0){
		...
		encOutData = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[0].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[1].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[2].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[3].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[4].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		encBuf[5].dataMem = (unsigned char*)malloc(picWidth*picHeight*3/2);
		pthread_create(&thread1_id, NULL, &pthreadEncoderHandle, NULL);
		pthread_create(&thread2_id, NULL, &pthreadCopyHandle, NULL);
		//pthread_create(&thread3_id, NULL, &pthreadSurfaceHandle, NULL);
		...
		exitEncPthreadFlag =1; //mask exit pthread
		exitSurPthreadFlag =1; //mask exit pthread
		exitCpyPthreadFlag =1; //mask exit pthread
		/* ######################################### */
			...
			release();
			free(encOutData);
			free(encBuf[0].dataMem);
			free(encBuf[1].dataMem);
			free(encBuf[2].dataMem);
			free(encBuf[3].dataMem);
			free(encBuf[4].dataMem);
			free(encBuf[5].dataMem);
...
static void* pthreadCopyHandle(void *arg)
{
		...
		while(1)
		{
			if(exitCpyPthreadFlag ==1)
			{
				exitCpyPthreadFlag =0;
				pthread_exit(NULL);
				break;
			}
...
/*static void* pthreadSurfaceHandle(void *arg)
{
		...
		while(1)
		{
			if(exitSurPthreadFlag ==1)
			{
				exitSurPthreadFlag =0;
				pthread_exit(NULL);
				break;
			}
